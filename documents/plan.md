...
- **State/Data Fetching**:
  ...
  - Dashboard first load reads directly from the normal granular query keys for
    accounts, profiles, calendars, tags, tasks, and the initial event window.
- **UI Layer**:
...
- **Data Layer**:
  ...
  - Calendar first load uses the same normal granular cache reads/invalidation
    model as web.
- **Auth**:
...
- **Query model**: Both web dashboard and native calendar now read directly from the existing granular query keys for accounts, profiles, calendars, events, tasks, and tags.
